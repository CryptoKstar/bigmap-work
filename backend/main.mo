...
shared (msg) actor class icdrive (){
  ...
  public shared(msg) func createCanister(userName: UserName) : async ?FileCanister {
    switch(user.findOne(msg.caller)){
      case null{
        ...
        user.createOne(msg.caller, userName, canId, userName);
  ...
  public shared(msg) func getUserId() : async ?Principal {
    return(?msg.caller)
  };
